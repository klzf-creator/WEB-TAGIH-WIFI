...
// --------------------------------------------------
const SUPABASE_URL = "https://huundmnefqeqbjlahisw.supabase.co"; // GANTI INI
const SUPABASE_ANON_KEY =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imh1dW5kbW5lZnFlcWJqbGFoaXN3Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzA2MDg0OTEsImV4cCI6MjA4NjE4NDQ5MX0.nvtdTmcsiZbD8IWR8PmY-rDwpPSSlda8L5FXJDLawfk"; // GANTI INI

// [FIX 1] Ganti nama variabel jadi 'db' agar tidak bentrok dengan library 'supabase' bawaan CDN
...
// [FIX 2] Gunakan 'window.App' agar bisa dibaca oleh file index.html (tombol onclick)
window.App = {
  // ============ INIT ============
  async init() {
    console.log("Aplikasi dimulai..."); // Debugging
  ...
  async loadCustomers() {
    try {
      // [FIX 3] Ganti 'supabase' menjadi 'db' di semua pemanggilan query
  ...
  async loadPayments() {
    ...
    try {
      ...
      localStorage.setItem(
        `tagihwarga_payments_${monthYear}`,
  ...
  renderVillages() {
    ...
    container.innerHTML = State.villages
      .map(
        (village) => `
            <button
  ...
  async selectVillage(village) {
    ...
    if (headerSub) headerSub.textContent = village;
  ...
  setFilter(filter) {
    ...
    if (filter === "unpaid") {
      btnUnpaid.className =
        "flex-1 py-2 text-sm font-semibold rounded-lg transition-all bg-white text-red-600 shadow-sm";
      btnAll.className =
        "flex-1 py-2 text-sm font-semibold rounded-lg transition-all text-gray-500";
    } else {
      btnAll.className =
        "flex-1 py-2 text-sm font-semibold rounded-lg transition-all bg-white text-blue-600 shadow-sm";
      btnUnpaid.className =
        "flex-1 py-2 text-sm font-semibold rounded-lg transition-all text-gray-500";
    }
  ...
  renderCustomerList() {
    ...
    if (statUnpaid) statUnpaid.textContent = unpaidCount;
    if (statTotal) statTotal.textContent = enriched.length;
  ...
  async showDetail(customerId) {
    ...
    // Check if proof image is expired
